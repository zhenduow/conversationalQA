...
with open('MSDialog-Complete.json') as f:
    ...
    for k in data.keys():
        ...
        if len(conversations) <= 2 or len(conversations) >= 10 or len(conversations) % 2 or len(conversations[-1]) > 512 :
            continue
        for convid in range(len(conversations)):
            conversations[convid] = conversations[convid][:max_len]

        out = open('MSDialog-Product/' + k, 'w')
        out.write('\n'.join(conversations))
        out.close()
